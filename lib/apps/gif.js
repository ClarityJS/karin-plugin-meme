import karin, { base64, common, segment } from 'node-karin';
import { Version } from '../common/index.js';
import { gif, Utils } from '../models/index.js';
export const slice = karin.command(/^#?(?:(清语)?表情|(?:clarity-)?meme)?gif分解$/, async (e) => {
    try {
        ...
        const imageSegments = await Promise.all(gifImages.map(async (frame) => segment.image(`base64://${await base64(frame)}`)));
        replyMessage = [...replyMessage, ...imageSegments, segment.text('===== 分解的图片 =====')];
        await e.bot.sendForwardMsg(e.contact, common.makeForward(replyMessage, e.bot.account.selfId, e.bot.account.name), {
            news: [{ text: 'Gif分解' }],
            prompt: 'Gif分解',
...
export const gearshift = karin.command(/^#?(?:(清语)?表情|meme(?:-plugin)?)?gif变速\s*(.+?)$/i, async (e) => {
