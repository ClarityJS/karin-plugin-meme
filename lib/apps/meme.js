 ...
 * 禁用表情检查
 */
const checkBlacklisted = async (keyword) => {
 ...
 * 防误触发处理
 */
const checkUserText = (min_texts, max_texts, UserText) => {
 ...
 * 表情包生成准备
 */
const validatePrepareMeme = async (e, keyword, memeKeyType, isPreset = false) => {
    ...
    /* 检查用户权限 */
    if (!checkUserAccess(e))
        return false;
    /* 检查禁用表情列表 */
    if (await checkBlacklisted(keyword))
        return false;
    /* 防误触发处理 */
    if (!checkUserText(min_texts, max_texts, UserText))
        return false;
    try {
        const Preset = isPreset ? { Preset: await Utils.Tools.getPreseInfo(keyword) } : {};
        const result = await Meme.make(e, memeKey, min_texts, max_texts, min_images, max_images, defText, args_type, UserText, isPreset, Preset);
 ...
 * 预设命令
 */
export const preset = karin.command(presetRegExp, async (e) => {
    if (!Config.meme.enable)
        return false;
