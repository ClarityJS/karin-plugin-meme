...
export const random = karin.command(/^#?(?:(清语)?表情|(?:clarity-)?meme)?随机(?:表情|meme)(包)?$/i, async (e) => {
        ...
        for (const memeKey of memeKeys) {
            ...
            if ((min_texts === 1 && max_texts === 1) ||
                ...
                try {
                    ...
                    const result = await Meme.make(e, memeKey, min_texts, max_texts, min_images, max_images, defText, args_type, '', false);
                    let replyMessage = [
