 ...
 * @returns  创建或更新后的记录对象
 */
export declare function add(name: string, key: string, arg_name: string, arg_value: string | number): Promise<[Model, boolean | null]>;
/**
 * 根据唯一指令标识符获取表情预设记录
 * @param  name - 唯一指令标识符（主键）
 ...
 * @returns  找到的记录对象数组
 */
export declare function getAllByKey(key: string): Promise<Model[]>;
/**
 * 根据参数名称获取所有表情预设记录
 * @param name - 参数名称
 * @returns  找到的记录对象数组
 */
export declare function getAllSelect(name: string): Promise<Model[]>;
/**
 * 根据唯一指令标识符删除表情预设记录
 * @param  name - 唯一指令标识符（主键）
 * @returns 删除的记录数量
